...
type Polynomial []*algebra.Integer
...
func NewPolynomialInts(coeffs ...int) (p Polynomial) {
	p = make([]*algebra.Integer, len(coeffs))
...
func NewPolynomial(coeffs []algebra.FieldElement) (p Polynomial) {
	p = make([]*algebra.Integer, len(coeffs))
	for i := 0; i < len(coeffs); i++ {
		p[i] = coeffs[i].Big()
	}
...
// over finite fields.
func (p Polynomial) Add(q Polynomial, m *algebra.Integer) Polynomial {
	if p.Compare(&q) < 0 {
		return q.Add(p, m)
	}
	var r Polynomial = make([]*algebra.Integer, len(p))
	for i := 0; i < len(q); i++ {
		a := new(big.Int)
		a.Add(p[i], q[i])
		r[i] = a
...
func (p Polynomial) Neg() Polynomial {
	var q Polynomial = make([]*algebra.Integer, len(p))
	for i := 0; i < len(p); i++ {
		b := new(big.Int)
		b.Neg(p[i])
		q[i] = b
...
func (p Polynomial) Clone(adjust int) Polynomial {
	var q Polynomial = make([]*algebra.Integer, len(p)+adjust)
	if adjust < 0 {
...
func (p *Polynomial) reduce(m *algebra.Integer) {
	if m == nil {
		return
	}
	for i := 0; i <= (*p).Degree(); i++ {
...
func (p Polynomial) Sub(q Polynomial, m *algebra.Integer) Polynomial {
	r := q.Neg()
	return p.Add(r, m)
}

// Mul computes P * Q
func (p Polynomial) Mul(q Polynomial, m *algebra.Integer) Polynomial {
	if m != nil {
		p.reduce(m)
		q.reduce(m)
	}
	var r Polynomial = make([]*algebra.Integer, p.Degree()+q.Degree()+1)
	for i := 0; i < len(r); i++ {
		r[i] = big.NewInt(0)
	}
	for i := 0; i < len(p); i++ {
...
// Div returns (P / Q, P % Q)
func (p Polynomial) Div(q Polynomial, m *algebra.Integer) (quo, rem Polynomial) {
	if m != nil {
		p.reduce(m)
		q.reduce(m)
	}
	...
	quo = make([]*algebra.Integer, p.Degree()-q.Degree()+1)
	rem = p.Clone(0)
	for i := 0; i < len(quo); i++ {
		quo[i] = big.NewInt(0)
	}
...
func (p Polynomial) GCD(q Polynomial, m *algebra.Integer) Polynomial {
	if p.Compare(&q) < 0 {
		return q.GCD(p, m)
	}
	if q.isZero() {
...
func (p Polynomial) Mod(q Polynomial, m *algebra.Integer) Polynomial {
...
func (p Polynomial) Quo(q Polynomial, m *algebra.Integer) Polynomial {
	quo, _ := p.Div(q, m)
	return quo
}

// Eval returns p(v) where v is the given big integer
func (p Polynomial) Eval(x *algebra.Integer, m *algebra.Integer) (y *algebra.Integer) {
	y = big.NewInt(0)
	accx := big.NewInt(1)
	xd := new(big.Int)
	for i := 0; i <= p.Degree(); i++ {
...
func (p Polynomial) Compose(q Polynomial, m *algebra.Integer) Polynomial {
...
func (p Polynomial) Pow(k *algebra.Integer, m *algebra.Integer) Polynomial {
	res := NewPolynomialInts(1)
	cur := p.Clone(0)

	for k.Cmp(algebra.FromInt64(0)) != 0 {
		if algebra.Mod(k, algebra.FromInt64(2)).Cmp(algebra.FromInt64(0)) != 0 {
			res = res.Mul(cur, m)
		}
		k.Rsh(k, 1)
		cur = cur.Mul(cur, m)
